...
export const TaskPrioritiesList = memo<TaskPrioritiesListProps>(props => {
  const { projectId, workspaceId } = props;

  const { data: originalPriorities } = useSuspenseQuery(
    getWorkspacePriorities(workspaceId, projectId)
  );
  ...
  const [editedTaskPriority, setEditedTaskPriority] = useState<WorkspaceTaskPriorityType | null>(
    null
  );
  const { mutate: updateProjectPrioritiesOrder, isPending: isUpdatingProjectPrioritiesOrder } =
    useUpdateProjectPrioritiesOrder();
  const { mutate: selectProjectPriority, isPending: isSelectingProjectPriority } =
    useSelectProjectPriority();
  ...
  const handleCreateTaskPriority = useCallback(
    (data: Omit<TaskPriorityType, 'id'> & { id?: TaskPriorityType['id'] | null }) => {
      const onSuccess = () => {
        setIsOpen(false);
      };
      ...
      if (data.id) {
        ...
        createTaskPriority(
          { task_priority: data, project_id: projectId },
          {
            onSuccess,
          }
        );
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h5 className="sm:text-md text-sm font-bold text-muted-foreground sm:ml-2">
          Task Priorities:
        </h5>
        <Button size="sm" variant="ghost" onClick={() => setIsOpen(true)}>
          <Icon name="add" size={18} />
          <span className="sm:block hidden">Add Task Priority</span>
